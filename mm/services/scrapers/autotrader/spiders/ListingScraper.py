...
class ListingScraper(scrapy.Spider):
    ...
    def extractData(self,response):
        ...
        try:
            ...
            scrapedData = None
            if scraperType == "normal":
                scrapedData = self.graphql.extractDataFromJson(jsonData)
            elif scraperType == "validator":
            
                scrapedData = self.graphql.extractValidatorDataFromJson(jsonData)
            
            if scrapedData != None:
                data["data"]["accountId"] = self.accountId
            
                data["data"]["websiteId"] = self.websiteId
                
                data["data"]["featuredId"] = self.featureId
                
                data["data"]["planId"] = self.planId
                
                data["data"]["priority"] = self.priority

                data["data"].update(scrapedData)

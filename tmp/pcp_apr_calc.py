class pcp_apr_calc():
  ...
  def create_query(self,data,row_id):
    final_list = []
    for item in data.items():
      final_list.append("`{}` = '{}'".format(item[0],item[1]))
    query = "UPDATE `fl_listings` SET {} WHERE ID = {};".format(",".join(final_list),row_id)
    return query

  def insert_pcp_apr_data_into_db(self,id,data,obj_master):
      calculated_ap = self.calculate_apr_pcp(int(data['price']),int(data['mileage']),int(data['built']))
      # for key in data:
      #   calculated_ap[key] = data[key]
      if "cal_price_from_file" in data.keys():
        calculated_ap["cal_price_from_file"] = data["cal_price_from_file"]
      query = self.create_query(calculated_ap,id)
      obj_master.obj_db.recCustomQuery(query)
      obj_master.obj_db.recUpdate("fl_listings",data,{"ID":id})
      print("apr pcp data inserted for -> {}".format(str(id)))
